...
let main = async (oldSessionId) => {
    ...
    MongoClient.connect(appConfig.mongo.url, async function(err, db) {        
        ...
        for(var i in rs){                          
	// const
            item = convertObjectIdToString(_.cloneDeep(rs[i]));
            try {
                if(typeof item.result === 'string') item.result = JSON.parse(item.result);

		item.result.title = item.title
		item.result.upload_time = item.finished_time
//	item.result.infor = {"1":"2"}
		console.log('aaaaaaaaaaaaaaaaaaaaaaaaaa', item.result);
        ...
        if(isReplay) {
            ...
            if(rs.length > 0) {
                ...
                fs.renameSync(`/home/ubuntu/mon2cas/logs/${sessionId}.running.sql`, `/home/ubuntu/mon2cas/logs/${sessionId}.${numPassed === rs.length ? 'passed' : 'failed'}.${numPassed}.${rs.length}.sql`);
            }else {
